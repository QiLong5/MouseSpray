...
void SplatmapModifyVertex_float(float3 inVertex, float3 inNormal, float4 inTangent, out float3 outVertex, out float3 outNormal)
{
    SHG_TerrainToMeshCurvedWorld_float(inVertex, inNormal, inTangent, outVertex, outNormal);
}

void SplatmapModifyVertex_half(float3 inVertex, float3 inNormal, float4 inTangent, out float3 outVertex, out float3 outNormal)
{
    SHG_TerrainToMeshCurvedWorld_float(inVertex, inNormal, inTangent, outVertex, outNormal);
}

void SplatmapAlphaClip_float(float2 uv)
{
    float clipValue = TerrainToMeshCalculateClipValue(uv);
    #if defined(_ALPHATEST_ON)
        clip(clipValue - 0.5);
    #endif
}

float3 SplatmapSafeNormalTS(float3 normalTS)
{
    ...
    normalTS.z = max(normalTS.z, 1e-3);
    return normalize(normalTS);
}

void SplatmapInitializeSurfaceData_float(float2 uv, out SurfaceData surfaceData)
{
    float3 albedo;
    float alpha;
    float3 normalTS;
    ...
    float smoothness;
    float occlusion;
    SHG_TerrainToMeshCalculateLayersBlend_float(float4(uv, 0.0, 0.0), albedo, alpha, normalTS, metallic, smoothness, occlusion);
    ...
    surfaceData.normalTS = SplatmapSafeNormalTS(normalTS);
