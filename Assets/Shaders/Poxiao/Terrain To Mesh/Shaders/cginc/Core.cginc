...
// URP compatibility: some versions don't expose IsGammaSpace().
#ifndef IsGammaSpace
    #if defined(UNITY_COLORSPACE_GAMMA)
        #define IsGammaSpace() true
    #else
...
{	
	...
	//URP Blend////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		//For 2/3-layer materials, ignore unused B/A channels in control map.
		//Otherwise residual values in compressed splat textures will steal weight to black layers and darken output.
		float4 activeLayerMask = float4(1, 1, 0, 0);
		#if defined(RENDER_LAYER_3)
			activeLayerMask = float4(1, 1, 1, 1);
		#elif defined(RENDER_LAYER_2)
			activeLayerMask = float4(1, 1, 1, 0);
		#endif
		splatmaps[0] *= activeLayerMask;
			...
			// Normalize
			float sumHeight = max(GetSumHeight(splatmaps[0]), 1e-6);
